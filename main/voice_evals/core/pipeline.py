...
try:
    from .diarization_stereo import StereoDiarizer, DiarizationResult
    USE_STEREO = True
except ImportError:
    USE_STEREO = False

try:
    from .diarization_vad import VADDiarizer
    USE_VAD = True
except ImportError:
    USE_VAD = False
...
class VoiceEvaluationPipeline:
    ...
    def __init__(self,
        ...
        if enable_diarization:
            try:
                # Try stereo diarization first (works without HF, perfect for stereo audio)
                if USE_STEREO and len(self.audio.shape) == 2:
                    print("Using stereo channel diarization (no HF models needed)...")
                    self.diarizer = StereoDiarizer()
                    self.diarizer_type = 'stereo'
                elif USE_VAD:
                    print("Using VAD-based diarization...")
                    self.diarizer = VADDiarizer(hf_token=hf_token)
                    self.diarizer_type = 'vad'
                elif USE_STANDARD:
                    print("Using standard diarization...")
                    self.diarizer = SpeakerDiarizer(
                        hf_token=hf_token,
                        min_speakers=num_speakers or 2,
                        max_speakers=num_speakers or 2
                    )
                    self.diarizer_type = 'standard'
                else:
                    raise ImportError("No diarization method available")
            except Exception as e:
                print(f"⚠️  Diarization disabled: {e}")
                self.enable_diarization = False
                self.diarizer_type = None
    ...
    def run_diarization(self) -> Optional[DiarizationResult]:
        ...
        try:
            if self.diarizer_type == 'stereo':
                self.diarization_result = self.diarizer.diarize(self.audio_path)
            elif self.diarizer_type == 'vad':
                self.diarization_result = self.diarizer.diarize(
                    self.audio_path,
                    num_speakers=self.num_speakers,
                    enable_vad_chunking=True
                )
            else:
                self.diarization_result = self.diarizer.diarize(
                    self.audio_path,
                ...
                if hasattr(self.diarizer, 'visualize_diarization'):
    ...
    def save_results(self, metrics: DiarizedVoiceMetrics, output_path: Optional[str] = None):
        ...
        if self.diarization_result and self.diarizer:
            ...
            if hasattr(self.diarizer, 'save_rttm'):
                self.diarizer.save_rttm(self.diarization_result, rttm_path)
            elif hasattr(self.diarizer, 'save_diarization_rttm'):
